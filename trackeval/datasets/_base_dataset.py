...
class _BaseDataset(ABC):
    ...
    @staticmethod
    def _calculate_box_ious(bboxes1, bboxes2, box_format='xywh', do_ioa=False, do_giou=False):
        """ Calculates the IOU (intersection over union)/GIOU between two arrays of boxes.
        ...
        if do_ioa:
            ...
            if not do_giou:
                ...
                # Calculate the smallest enclosing box's area
                enc_xmin = np.minimum(bboxes1[:, None, 0], bboxes2[:, 0])
                enc_ymin = np.minimum(bboxes1[:, None, 1], bboxes2[:, 1])
                enc_xmax = np.maximum(bboxes1[:, None, 2], bboxes2[:, 2])
                enc_ymax = np.maximum(bboxes1[:, None, 3], bboxes2[:, 3])
                enc = (enc_xmax - enc_xmin) * (enc_ymax - enc_ymin)
                gious = ious - (enc - union) / enc
                return 0.5*(1+gious)

    @staticmethod
    def _calculate_box_ious_3d(bboxes1, bboxes2, box_format='xyzwdh', do_giou=False):
        """ Calculates the IOU (intersection over union)/GIOU between two arrays of boxes.
        Allows variable box formats ('xyzwdh' and 'x0y0z0x1y1z1').
        If do_ioa (intersection over area) , then calculates the intersection over the area of boxes1 - this is commonly
        used to determine if detections are within crowd ignore region.
        """
        if box_format in 'xyzwdh':
            # layout: (x0, y0, w, h)
            bboxes1 = deepcopy(bboxes1)
            bboxes2 = deepcopy(bboxes2)

            bboxes1[:, 3] = bboxes1[:, 0] + bboxes1[:, 3]
            bboxes1[:, 4] = bboxes1[:, 1] + bboxes1[:, 4]
            bboxes1[:, 5] = bboxes1[:, 2] + bboxes1[:, 5]

            bboxes2[:, 3] = bboxes2[:, 0] + bboxes2[:, 3]
            bboxes2[:, 4] = bboxes2[:, 1] + bboxes2[:, 4]
            bboxes2[:, 5] = bboxes2[:, 2] + bboxes2[:, 5]

        elif box_format not in 'x0y0z0x1y1z1':
            raise (TrackEvalException('box_format %s is not implemented' % box_format))

        # Intersection
        min_corner = np.maximum(bboxes1[:, None, :3], bboxes2[:, :3])
        max_corner = np.minimum(bboxes1[:, None, 3:], bboxes2[:, 3:])
        inter_dims = np.maximum(0.0, max_corner - min_corner)
        intersection = np.prod(inter_dims, axis=2)

        # Box volumes
        boxes1_volume = np.prod(bboxes1[:, 3:] - bboxes1[:, :3], axis=1)
        boxes2_volume = np.prod(bboxes2[:, 3:] - bboxes2[:, :3], axis=1)

        # Union
        union = boxes1_volume[:, None] + boxes2_volume - intersection
        ious = intersection / union
        if not do_giou:
            return ious
        else:
            # Enclosing box
            enclosing_min_corner = np.minimum(bboxes1[:, None, :3], bboxes2[:, :3])
            enclosing_max_corner = np.maximum(bboxes1[:, None, 3:], bboxes2[:, 3:])
            enclosing_dims = enclosing_max_corner - enclosing_min_corner
            enclosing_volume = np.prod(enclosing_dims, axis=2)
            gious = ious - (enclosing_volume - union) / enclosing_volume
            return 0.5 * (1 + gious)

    @staticmethod
    def _calculate_euclidean_similarity(dets1: object, dets2: object, zero_distance: object = 2.0) -> object:
