...
class CMC3DBox(_BaseDataset):
    """Dataset class for Curtin Multi-Camera 3D bounding box tracking"""
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                    ...
                    if is_gt:
                        ...
                        raise TrackEvalException(
                            'Cannot convert tracking data from tracker %s, sequence %s to float. Is data corrupted?' % (
                                tracker, seq))
                try:
                    raw_data['dets'][t] = np.atleast_2d(time_data[:, 2:8])
                ...
                raw_data['classes'][t] = np.atleast_1d(time_data[:, 9]).astype(int)

                if is_gt:
                    gt_extras_dict = {'zero_marked': np.atleast_1d(time_data[:, 8].astype(int))}
                    raw_data['gt_extras'][t] = gt_extras_dict
                else:
                    raw_data['tracker_confidences'][t] = np.atleast_1d(time_data[:, 8])
            else:
                raw_data['dets'][t] = np.empty((0, 6))
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # Do not do anything to the tracker data
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        if self.config['BASE_METRIC'] == 'IOU':
            similarity_scores = self._calculate_box_ious_3d(gt_dets_t, tracker_dets_t, box_format='xyzwdh',
                                                            do_giou=False)
        elif self.config['BASE_METRIC'] == 'GIOU':
            similarity_scores = self._calculate_box_ious_3d(gt_dets_t, tracker_dets_t, box_format='xyzwdh',
                                                            do_giou=True)
        elif self.config['BASE_METRIC'] == 'EUCLIDEAN':
            # get the Euclidean similarity between the center of the bounding boxes
            center_gt = deepcopy(gt_dets_t)
            center_tracker = deepcopy(tracker_dets_t)
            center_gt[:, 0] = center_gt[:, 0] + center_gt[:, 3] / 2
            center_gt[:, 1] = center_gt[:, 1] + center_gt[:, 4] / 2
            center_gt[:, 2] = center_gt[:, 2] + center_gt[:, 5] / 2
            center_gt = center_gt[:, :3]
            center_tracker[:, 0] = center_tracker[:, 0] + center_tracker[:, 3] / 2
            center_tracker[:, 1] = center_tracker[:, 1] + center_tracker[:, 4] / 2
            center_tracker[:, 2] = center_tracker[:, 2] + center_tracker[:, 5] / 2
            center_tracker = center_tracker[:, :3]
            similarity_scores = self._calculate_euclidean_similarity(center_gt, center_tracker)
